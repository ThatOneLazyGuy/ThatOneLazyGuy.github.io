...
As a second year student at Buas (Breda University of Applied Sciences) I have been working on a job system for that past 8 weeks to more easily multi-thread parts of a game engine.

In this blog we are going to create a job system which lets you easily turn any function into a job to be performed on a different thread, we'll be using many useful feature of the c++17 standard and creating a simple API to get be able to work with multiple threads quickly and easily.
...
To get our how many threads we should create we use `std::thread::hardware_concurrency()` this gives us a hint to the logical core count of our CPU (Logical cores might also be called hardware threads, since this can cause confusing between software threads I will be calling them logical cores). Now we can create a thread for each logical core, maximizing the thread count without the threads fighting for time on the logical cores of our CPU.
...
Now we have our pool of thread based on our logical core count, we might think we are doing but unfortunately there is one more problem we want to solve: \
**context switching.**\
Threads are managed by the OS to decide when on on which logical core to run them, this means that a thread and its context can be moved from one logical core to another by the OS based on which logical has time.
Since we want our threads to be most affectively take advantage of all our logical cores we want to tie each thread to one of those logical core. Unfortunately there is no standardized way to do this, but since I am using windows my code would look something like this:
...
1. Storing function with different return types or parameters in the same list.
...
4. Knowing which task have finished executing.

Lets start at the beginning with problems 1 and 2, where we can solve these problem simply by not storing any of this information directly, but instead storing lambda:
...
We use a lambda  in order to capture the function and its arguments which will allow use to execute this later, since the lambda doesn't take any arguments anymore nor does it return anything we can simply describe it as be of the type `std::function<void()>`, allow use to store different tasks together in one list.
...
A promise works together with `std::future<>` to deliver a *promised* value in the *future*. In our case we first want to describe the type of promised value, this will be our return value type.
...
To combine these two solutions together we have to keep a few thing in mind however. We need to get the future object back from the promise before the lambda captures it. The lambda needs to capture the promise in order to fulfil the promise by setting its value to the return value of the function. The only problem is that std::promise is non-moveable and can't be captured by the lambda after we created it, that is why instead of creating a new object on the stack we create a shared_ptr of a promise that we can then capture with the lambda.
...
Since we can now store out tasks in a single list, lets do that. For out purpose the list type `std::queue<>`, this allows use to use a FIFO (First In First Out) structured where the first task we added also gets picked up by a worker thread first. 
To added job to the queue we can use our earlier code to create a function called "AddToQueue":
...
To fill in the missing pieces of my earlier example I've made the AddToQueue function templated to allow for different function types and arguments. Template types are also use with `std::invoke_result_t<>` to get back the explicit return type of that function to use for the promise and future. Another possibly confusing addition is to the lambda function: `if constexpr (std::is_same_v<ReturnType, void>)`, but all this does is check if the ReturnType is equal to void at runtime, it does this in order to change how to call `promise->set_value()` since a promise with type void means there is no actual value to set and thus the function is required to have no arguments.

Since we are working with a queue being accessed by multiple threads we want to use an `std::mutex` in the JobManager to make sure the threads do not try to access the shared queue simultaneously when they aren't suppose to.
To final adding of the task to the queue happens using QueueTask() which acquires a lock on the mutex to push the task the queue and then return which automatically releases the lock.
...
Now that we have some tasks and some threads to perform them, we can describe how exactly they will be performing their tasks. A worker thread doesn't do much when no tasks are available it should simply remain idle and wait for the JobManger to notify it about tasks becoming available, this way no CPU usage is wasted polling the queue for new tasks constantly.
A convenient way to do both of these things is to use an `std::condition_variable`. A condition_variable can be used to check if a certain condition is true, continue if it is, or block the thread if it isn't true yet. To notify when the condition_variable should check its condition, we can use: `condition_variable::notify_one()` to notify a single thread to or: `condition_variable::notify_all()` to notify all threads.\
\
To start of we want to have a loop for that uses the condition_variable to check if new work is available. The wait function on the condition_variable uses a lock to check if a predicate is true and continues if it is.
Out predicate will check two things:
1. If the queue isn't empty.
2. If the new variable m_shouldExit is true, if this is true the worker thread should return.

If either are true the condition_variable will continue.
...
Although, there are some limitations that we might want to solve.

### Grouping work

When you want to multithread something, a lot of the time this is because you have a large number of thing to process. A large number of tasks to process currently requires you to add a lot of jobs to the queue by using `AddToQueue`, this can be quite slow since the threads have to frequently try to access the lock to grab a new task from the queue, causing a lot of contention.
...
The dispatch function is designed to work on any container type (or pointer type) and will create groups of function with the first parameter being of the same type as the container type. Parameters of the function are the begin iterator for any container type which can be iterated through one by one, the second parameter is the amount of elements in the container then followed by the parameters for the function to execute for these tasks and the extra arguments.
...
A lambda function is made which loops through each of the iterators in its group and executes the function for that element and the extra arguments, after that the lambda sets the task group's promise so the future is notified about the task having finished. The task group is then added to the queue:
...
`AddToQueue` and `Dispatch` return one ore more future objects, these can be used to get the values back from out tasks.

`future.wait()`: This method can be used to block this thread to wait for the promise to be fulfilled by our task.
`future.get()`: This method also blocks this thread and wait for the promise to be fulfilled. However it also returns back the value that the promise was set to, marking the future as invalid afterwards.

#### Finished?

While getting the values and waiting for them is useful of course, what if we simply want to check if the task is done without blocking this thread? Unfortunately there isn't a simple method in std::future to do this, luckily we can do it with a bit of trickery.
...
In the `IsFinished` function we wait for 0 seconds, which will tell us the state of the future after we finished waiting for no time. If the state of the future is ready, that means its finished.
Now that we know the state of out future, we can chose to only get our value from it back if its true, otherwise we can simply let the rest of the program run. This type of check can be useful for when something is being loaded in on a worker thread and you want to show that it is still running, or if you are streaming something in and only want to process it during the frame that it finishes but continue otherwise.
...
A lot of different program require multithreading and a job system that can be reused in different ways can be a useful tool to improve performance or add more features without much trouble.

I hope this blog was helpful in learning more about job system and can help you more effectively work with multiple threads!